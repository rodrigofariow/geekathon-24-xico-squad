...
async function sendImageToClaude(base64File: string, formatType: string) {
    try {
        ...
        return result.wines == undefined ? JSON.parse(msg.content[0].text) : JSON.parse(msg.content[0].text).wines
...
export async function getAllBottlesFromImage(originalImage: VivinoImgMeta,) {
    try {
        ...
        if (!originalImage.fileExtension || !allowedTypes.includes(originalImage.fileExtension)) {
            throw new Error(
                `Invalid file type. Only ${allowedTypes.join(", ")} files are allowed.`
            );
        }

        return await sendImageToClaude(originalImage.base64, originalImage.fileExtension);
    } catch (error) {
