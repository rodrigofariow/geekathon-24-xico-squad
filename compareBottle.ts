...
async function sendImagesToClaude(originalImagePath, originalImageName, potentialImagePaths, potentialImageNames) {
    ...
    try {
        ...
        const potentialImages = potentialImagePaths.map((imagePath, index) => {
            ...
            return {data: buffer.toString('base64'), fileName: potentialImageNames[index]};
        });
            ...
            messages: [
                {
                    ...
                    content: [
                        ...
                        {
                            type: "text",
                            text: `The original image is "${originalImageName}". Below are potential images to compare. Their file names are:\n\n` +
                                potentialImages.map(({fileName}, index) => `${index + 1}. ${fileName}`).join("\n") +
...
async function compare(originalImagePath, ...imagePaths) {
    ...
    const potentialImageNames = imagePaths.map((imagePath) => path.basename(imagePath));
